...
public class Skill {
	...
	public Result add(com.lingo.profiles.bean.Skill data)
	{
		...
		try
		{
			String sql = "insert into Profiles.Skill(PID,Title,Content) values(?,?,?);";
	...
	public Result update(com.lingo.profiles.bean.Skill data)
	{
		...
		try
		{
			String sql = "update Profiles.Skill set Title=?,Content=? where ID=?";
	...
	public ListResult<com.lingo.profiles.bean.Skill> getList(com.lingo.profiles.bean.Skill data)
	{
		...
		try {
			...
			while (rs.next()) {				
				...
				com.lingo.profiles.bean.Skill skill = new com.lingo.profiles.bean.Skill(id, data.getPid(), title, content);
				list.add(skill);
			}
			rs.close();
			result.setResult(1);
